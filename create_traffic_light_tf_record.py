...
def create_traffic_light_tf_record(encoded_tl_image_data):
    """Creates a tf.Example proto from sample cat image.

      Args:
        encoded_tl_image_data: The jpg encoded data of the traffic light image.
    ...
    filename = encoded_tl_image_data['path'] # Filename of the image. Empty if image is not from file
    filename = filename.encode()

    with tf.gfile.GFile(encoded_tl_image_data['path'], 'rb') as fid:
    ...
    for box in encoded_tl_image_data['boxes']:
...
def main(_):
    ...
    for example in examples:
        tf_example = create_traffic_light_tf_record(example)
